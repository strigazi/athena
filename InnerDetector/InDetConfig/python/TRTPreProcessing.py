...
def InDetPRD_MultiTruthMakerTRTCfg(flags, name = "InDetTRT_PRD_MultiTruthMaker", **kwargs):
	...
	acc.addEventAlgo(CompFactory.InDet__PRD_MultiTruthMaker(name, **kwargs))
	return acc
...
def InDetTRT_DriftFunctionToolCfg(flags, useTimeInfo, usePhase, name = "InDetTRT_DriftFunctionTool", **kwargs):
	...
	if True: ######decide later what flag to be used here: TRT_on from ConfiguredNewTrackingCuts.py
...
def TRT_DriftCircleToolCfg(flags, useTimeInfo, usePhase, prefix, name = "InDetTRT_DriftCircleTool", **kwargs):
	...
	#
	if usePhase:
		InDet__TRT_DriftCircleTool = CompFactory.TRT_DriftCircleToolCosmics
	else:
		InDet__TRT_DriftCircleTool = CompFactory.TRT_DriftCircleTool
	...
	kwargs.setdefault("MaskFirstHTBit"       , False)
	kwargs.setdefault("MaskMiddleHTBit"       , False)
	kwargs.setdefault("MaskLastHTBit"       , False)
	...
	kwargs.setdefault("MaskFirstHTBitArgon"       , False)
	kwargs.setdefault("MaskMiddleHTBitArgon"       , False)
	kwargs.setdefault("MaskLastHTBitArgon"       , False)
	...
	acc.setPrivateTools(InDet__TRT_DriftCircleTool(name, **kwargs))
	return acc
...
def InDetTRT_RIO_MakerCfg(flags, useTimeInfo, usePhase, prefix, collection, name = "InDetTRT_RIO_Maker", **kwargs):
	...
	acc.addEventAlgo(CompFactory.InDet__TRT_RIO_Maker(name, **kwargs))
	return acc
...
def InDetTRT_RIO_MakerPUCfg(flags, useTimeInfo, usePhase, prefix, collectionPU, name = "InDetTRT_RIO_MakerPU", **kwargs):
	...
	# --- TRT_DriftCircleTool
	#
	sub_acc, InDetTRT_DriftCircleTool = TRT_DriftCircleToolCfg(flags, useTimeInfo, usePhase, prefix, name = "InDetTRT_DriftCircleTool")
	acc.merge(sub_acc)
	acc.addPublicTool(InDetTRT_DriftCircleTool)
	#
	# --- TRT_RIO_Maker Algorithm
...
if __name__ == "__main__":
	...
	if not ConfigFlags.InDet.doDBMstandalone: ##and TRT_on from ConfiguredNewTrackingCuts.py
		from TRT_GeoModel.TRT_GeoModelConfig import TRT_GeometryCfg
		acc = TRT_GeometryCfg( ConfigFlags )
		acc.merge(TRTPreProcessingCfg(ConfigFlags,(not ConfigFlags.InDet.doTRTPhaseCalculation or ConfigFlags.Beam.Type =="collisions"),False))
		top_acc.merge(acc)
	iovsvc = top_acc.getService('IOVDbSvc')
	iovsvc.OutputLevel=5
	top_acc.run(25)
	##top_acc.store(open("test_TRTPrepocessing.pkl", "w")) ##uncomment to store top_acc into pkl file